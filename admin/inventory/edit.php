                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <?php 
                                $id=$_GET['id'];
                                $queryEditItem=mysqli_query($connection, "SELECT * FROM items WHERE id='$id'");
                                foreach ($queryEditItem as $data)
                            ?>
                            <form action="update.php" method="POST">
                                <input type="hidden" class="form-control" id="editItemId" name="editItemId" value="<?php echo $data['id']?>" required>
                                <div class="mb-3">
                                    <label for="editItemName" class="form-label">Item Name</label>
                                    <input type="" class="form-control" id="editItemName" name="editItemName" placeholder="Item Name" value="<?php echo $data['name']?>" required>
                                </div>
                                <div class="mb-3">
                                    <label for="editItemCategory" class="form-label">Category</label>
                                    <select class="form-control" id="editItemCategory" name="editItemCategory"  aria-label="Default select example" value="<?php echo $data['itemcategory_id']?>" required>
                                        <option selected>Select Category</option>
                                        <?php
                                            while ($row = mysqli_fetch_array($queryInventoryCategory)){
                                                if($row['id'] == $data['itemcategory_id']){
                                ...
                                <div class="mb-3">
                                    <label for="editItemGood" class="form-label">Good Quantity</label>
                                    <input type="number" class="form-control" id="editItemGood" name="editItemGood" placeholder="Good Quantity" step="1" value="<?php echo $data['good']?>" required>
                                </div> 
                                <div class="mb-3">
                                    <label for="editItemBad" class="form-label">Bad Quantity</label>
                                    <input type="number" class="form-control" id="editItemBad" name="editItemBad" placeholder="Bad Quantity" step="1" value="<?php echo $data['bad']?>" required>
                                </div>                              
